...
@Component({
  ...
  template: `
    <div
      class="relative z-10 bg-white rounded-2xl shadow-2xl p-4 sm:p-6 md:p-8 max-w-lg w-full flex flex-col items-center max-h-[90vh] overflow-y-auto"
    >
            ...
            <div class="flex flex-col items-center">
              <svg width="80" height="80" viewBox="0 0 80 80">
                <ellipse 
                  cx="40" 
                  cy="45" 
                  [attr.rx]="faceShape().rx" 
                  [attr.ry]="faceShape().ry" 
                  [attr.fill]="colorPiel()"
                ></ellipse>
                <ellipse 
                  cx="40" 
...
export class HomeComponent implements OnInit {
  ...
  // Calcula la forma de la cara según la selección
  faceShape() {
    switch (this.formaCara()) {
      case 'gordo':
        return { rx: 26, ry: 28 };
      case 'flaco':
        return { rx: 18, ry: 32 };
  ...
  onSubmit(event: Event) {
    ...
    if (newUser) {
      ...
      // Aquí puedes redirigir al juego o mostrar otra vista
  ...
  private resetForm() {
    this.nickname.set('');
    this.genero.set('masculino');
    this.colorPiel.set('#fcd7b6');
    this.colorPelo.set('#3e2723');
    this.formaCara.set('normal');
  }
}
